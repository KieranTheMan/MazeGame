...
const MazeGrid: React.FC<MazeGridProps> = ({
  ...
  const renderCell = (cellValue: string, row: number, col: number) => {
    ...
    return (
      <div
        key={`${row}-${col}`} // React key for list rendering (unique per cell)
        className={`
          w-12 h-12 border border-gray-300 flex items-center justify-center text-xs font-bold
          ${
            isWall ? "bg-amber-800" : "bg-white"
          }    // Dark brown for walls, white for paths
          ${
            isPortal ? "bg-purple-200" : ""
          }          // Light purple background for portals
        `}
      >
